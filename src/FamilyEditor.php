...
<form method="post" action="FamilyEditor.php?FamilyID=<?php echo $iFamilyID ?>">
	...
	<div class="box box-info clearfix">
		<div class="box-header">
			<h3 class="box-title"><?= gettext("Información de la Familia") ?></h3>
			<div class="pull-right"><br/>
				<input type="submit" class="btn btn-primary" value="<?= gettext("Guardar") ?>" name="FamilySubmit"  class="form-control">
			</div>
		</div><!-- /.box-header -->
		<div class="box-body">
			<div class="form-group">
				<div class="row">
					<div class="col-md-6">
						<label><?= gettext("Nombre Familia:") ?></label>
				...
				<div class="row">
					<div class="col-md-6">
						<label><?= gettext("Dirección (Calle #Núm):") ?></label>
					...
					<div class="col-md-6">
						<label><?= gettext("Dirección (Bloque, Depto, Letra):") ?></label>
	...
	<div class="box box-info clearfix">
		<div class="box-header">
			...
			<div class="pull-right"><br/>
				<input type="submit" class="btn btn-primary" value="<?= gettext("Guardar") ?>" name="FamilySubmit" >
			</div>
		</div><!-- /.box-header -->
		<div class="box-body">
			<div class="row">
				<div class="form-group col-md-6">
					<label><?= gettext("Teléfono Casa:") ?></label>
					<div class="input-group">
						<div class="input-group-addon">
							<i class="fa fa-phone"></i>
						</div>
						<input type="text" Name="HomePhone" value="<?= htmlentities(stripslashes($sHomePhone)) ?>" size="30" maxlength="30" class="form-control" data-inputmask='"mask": "(+56) 9 9999 9999"' data-mask>
						<input type="checkbox" name="NoFormat_HomePhone" value="1" <?php if ($bNoFormat_HomePhone) echo " checked";?>><?= gettext("Do not auto-format") ?>
					</div>
				</div>
				<div class="form-group col-md-6">
					<label><?= gettext("Teléfono Trabajo:") ?></label>
					<div class="input-group">
						<div class="input-group-addon">
							<i class="fa fa-phone"></i>
						</div>
						<input type="text" name="WorkPhone" value="<?= htmlentities(stripslashes($sWorkPhone)) ?>" size="30" maxlength="30" class="form-control" data-inputmask='"mask": "(+56) 9 9999 9999"' data-mask/>
						<input type="checkbox" name="NoFormat_WorkPhone" value="1" <?= $bNoFormat_WorkPhone ? " checked" : ''?>><?= gettext("Do not auto-format") ?>
					</div>
				</div>
				<div class="form-group col-md-6">
					<label><?= gettext("Teléfono Móvil:") ?></label>
					<div class="input-group">
						<div class="input-group-addon">
							<i class="fa fa-phone"></i>
						</div>
						<input type="text" name="CellPhone" value="<?= htmlentities(stripslashes($sCellPhone)) ?>" size="30" maxlength="30" class="form-control" data-inputmask='"mask": "(+56) 9 9999 9999"' data-mask>
			...
			<div class="row">
				<div class="form-group col-md-6">
					<label><?= gettext("Correo Electrónico:") ?></label>
					<div class="input-group">
						<div class="input-group-addon">
				...
				<div class="form-group col-md-4">
					<label><?= gettext("¿Envíar Boletín?") ?></label><br/>
	...
	<div class="box box-info clearfix">
		<div class="box-header">
			<h3 class="box-title"><?= gettext("Otra Información") ?></h3>
			<div class="pull-right"><br/>
				<input type="submit" class="btn btn-primary" value="<?= gettext("Guardar") ?>" name="FamilySubmit">
			</div>
		</div><!-- /.box-header -->
		<div class="box-body">
			<?php if (!$bHideWeddingDate) { /* Wedding Date can be hidden - General Settings */
				...
				<div class="row">
					<div class="form-group col-md-4">
						<label><?= gettext("Fecha Matrimonio:") ?></label>
			...
			<div class="row">
				<?php if ($_SESSION['bCanvasser']) { // Only show this field if the current user is a canvasser ?>
					<div class="form-group col-md-4">
						<label><?= gettext("¿Está Bien Visitar?") ?> </label><br/>
	...
	<div class="box box-info clearfix">
		<div class="box-header">
			<h3 class="box-title"><?= gettext("Información de Sobre") ?></h3>
			<div class="pull-right"><br/>
				<input type="submit" class="btn btn-primary" value="<?= gettext("Guardar") ?>" name="FamilySubmit" >
			</div>
		</div><!-- /.box-header -->
		<div class="box-body">
			<div class="row">
				<div class="form-group col-md-4">
					<label><?= gettext("Número Sobre:") ?></label>
	...
	<div class="box box-info clearfix">
		<div class="box-header">
			<h3 class="box-title"><?= gettext("Miembros Familia") ?></h3>
			<div class="pull-right"><br/>
				<input type="submit" class="btn btn-primary" value="<?= gettext("Guardar") ?>" name="FamilySubmit">
			</div>
		</div><!-- /.box-header -->
	...
	<tr>
		...
		<div class="MediumText">
			<center><?= $iFamilyID < 0 ? gettext("Puede crear miembros de la familia ahora o añadirlos después. Todas las entradas se transforman en <i>nuevos</i> registros de personas.") : '' ?></center>
		</div><br><br>
		<table cellpadding="3" cellspacing="0" width="100%">
		<thead>
		<tr class="TableHeader" align="center">
			<th><?= gettext("Primer N") ?></th>
			<th><?= gettext("Segundo N") ?></th>
			<th><?= gettext("Apellido") ?></th>
			<th><?= gettext("Sufijo") ?></th>
			<th><?= gettext("Sexo") ?></th>
			<th><?= gettext("Rol") ?></th>
			<th><?= gettext("Mes") ?></th>
			<th><?= gettext("Día") ?></th>
			<th><?= gettext("Año") ?></th>
			<th><?= gettext("Clasificación") ?></th>
		</tr>
		</thead>
		<?php
		...
		<tr>
			...
			<td class="TextColumn">
				<select name="Gender<?php echo $iCount ?>">
					<option value="0" <?php if ($aGenders[$iCount] == 0) echo "selected" ?> ><?= gettext("Seleccionar Sexo") ?></option>
					<option value="1" <?php if ($aGenders[$iCount] == 1) echo "selected" ?> ><?= gettext("Masculino") ?></option>
					<option value="2" <?php if ($aGenders[$iCount] == 2) echo "selected" ?> ><?= gettext("Femenino") ?></option>
				</select>
			</td>

			<td class="TextColumn">
				<select name="Role<?php echo $iCount ?>">
					<option value="0" <?php if ($aRoles[$iCount] == 0) echo "selected" ?> ><?= gettext("Seleccionar Rol") ?></option>
				<?php
			...
			<td class="TextColumn">
				<select name="BirthMonth<?php echo $iCount ?>">
					<option value="0" <?php if ($aBirthMonths[$iCount] == 0) { echo "selected"; } ?>><?= gettext("Desconocido") ?></option>
					<option value="01" <?php if ($aBirthMonths[$iCount] == 1) { echo "selected"; } ?>><?= gettext("Enero") ?></option>
					<option value="02" <?php if ($aBirthMonths[$iCount] == 2) { echo "selected"; } ?>><?= gettext("Febrero") ?></option>
					<option value="03" <?php if ($aBirthMonths[$iCount] == 3) { echo "selected"; } ?>><?= gettext("Marzo") ?></option>
					<option value="04" <?php if ($aBirthMonths[$iCount] == 4) { echo "selected"; } ?>><?= gettext("Abril") ?></option>
					<option value="05" <?php if ($aBirthMonths[$iCount] == 5) { echo "selected"; } ?>><?= gettext("Mayo") ?></option>
					<option value="06" <?php if ($aBirthMonths[$iCount] == 6) { echo "selected"; } ?>><?= gettext("Junio") ?></option>
					<option value="07" <?php if ($aBirthMonths[$iCount] == 7) { echo "selected"; } ?>><?= gettext("Julio") ?></option>
					<option value="08" <?php if ($aBirthMonths[$iCount] == 8) { echo "selected"; } ?>><?= gettext("Agosto") ?></option>
					<option value="09" <?php if ($aBirthMonths[$iCount] == 9) { echo "selected"; } ?>><?= gettext("Septiembre") ?></option>
					<option value="10" <?php if ($aBirthMonths[$iCount] == 10) { echo "selected"; } ?>><?= gettext("Octubre") ?></option>
					<option value="11" <?php if ($aBirthMonths[$iCount] == 11) { echo "selected"; } ?>><?= gettext("Noviembre") ?></option>
					<option value="12" <?php if ($aBirthMonths[$iCount] == 12) { echo "selected"; } ?>><?= gettext("Diciembre") ?></option>
				</select>
			</td>
			<td class="TextColumn">
			...
			<td>
				<select name="Classification<?php echo $iCount ?>">
					<option value="0" <?php if ($aClassification[$iCount] == 0) echo "selected" ?>><?= gettext("Sin Asignar") ?></option>
	...
	echo "<input type=\"submit\" class=\"btn\" value=\"" . gettext("Guardar") . "\" Name=\"FamilySubmit\">";
	if ($_SESSION['bAddRecords']) { echo "<input type=\"submit\" class=\"btn\" value=\"Guardar y Añadir\" name=\"FamilySubmitAndAdd\">"; }
	echo "<input type=\"button\" class=\"btn\" value=\"" . gettext("Cancelar") . "\" Name=\"FamilyCancel\"";
	if ($iFamilyID > 0)

...
<div class="row">
  <div class="col-lg-3 col-md-3 col-sm-3">
      ...
      <div class="box-body box-profile">
        ...
        <p class="text-muted text-center">
          ...
          if ($per_MembershipDate) {
            echo " desde: " . FormatDate($per_MembershipDate, false);
          } ?>
        </p>
        <?php if ($bOkToEdit) { ?>
          <a href="PersonEditor.php?PersonID=<?= $per_ID ?>" class="btn btn-primary btn-block"><b><?php echo gettext("Editar"); ?></b></a>
        <?php } ?>
      </div>
      <!-- /.box-body -->
    </div>
    ...
    <div class="box box-primary">
      <div class="box-header with-border">
        <h3 class="box-title text-center"><?php echo gettext("Sobre Mí"); ?></h3>
      </div>
      <!-- /.box-header -->
      <div class="box-body">
        <ul class="fa-ul">
          <li><i class="fa-li fa fa-group"></i><?php echo gettext("Familia:"); ?> <span>
							<?php
              if ($fam_ID != "") { ?>
              ...
              <?php } else
                echo gettext("(Sin Asignada)");
              ?>
						</span></li>
          <li><i class="fa-li glyphicon glyphicon-home"></i><?php echo gettext("Dirección:"); ?> <span>
						<a href="http://maps.google.com/?q=<?= $plaintextMailingAddress ?>" target="_blank">
              <?= $formattedMailingAddress ?>
            </a>
						</span></li>
          <?php if ($dBirthDate) { ?>
            <li><i class="fa-li fa fa-calendar"></i><?= gettext("Fecha Nac:") ?> <span><?= $dBirthDate ?></span> (<?php PrintAge($per_BirthMonth, $per_BirthDay, $per_BirthYear, $per_Flags); ?>)</li>
          <?php }
          if (!$bHideFriendDate) { /* Friend Date can be hidden - General Settings */ ?>
            <li><i class="fa-li fa fa-tasks"></i><?= gettext("Fecha Visita:") ?> <span><?= FormatDate($per_FriendDate, false) ?></span></li>
          <?php }
          if ($sCellPhone) { ?>
            <li><i class="fa-li fa fa-mobile-phone"></i><?= gettext("Móvil:") ?> <span><?= $sCellPhone ?></span></li>
          <?php }
          if ($sHomePhone) {
            ?>
            <li><i class="fa-li fa fa-phone"></i><?= gettext("Casa:") ?> <span><?= $sHomePhone ?></span></li>
            <?php
          }
          if ($sEmail != "") { ?>
            <li><i class="fa-li fa fa-envelope"></i><?= gettext("Correo:") ?> <span><a href="mailto:<?= $sUnformattedEmail ?>"><?= $sEmail ?></a></span></li>
          ...
          if ($sWorkPhone) {
            ?>
            <li><i class="fa-li fa fa-phone"></i><?= gettext("Trabajo:") ?> <span><?= $sWorkPhone ?></span></li>
          <?php } ?>
          <?php if ($per_WorkEmail != "") { ?>
            <li><i class="fa-li fa fa-envelope"></i><?= gettext("Otro Correo:") ?> <span><a href="mailto:<?= $per_WorkEmail ?>"><?= $per_WorkEmail ?></a></span></li>
            <?php if ($mailchimp->isActive()) { ?>
  ...
  <div class="col-lg-9 col-md-9 col-sm-9">
    <div class="box box-primary box-body">
      <?php if ($bOkToEdit) { ?>
        <a href="#" class="btn btn-app" data-toggle="modal" data-target="#upload-image"><i class="fa fa-camera"></i><?= gettext("Subir Fotografía") ?></a>
        <?php if ($personService->getUploadedPhoto($iPersonID) !== "") { ?>
          <a class="btn btn-app bg-orange" href="#" data-toggle="modal" data-target="#confirm-delete-image"><i class="fa fa-remove"></i> <?= gettext("Borrar Fotografía") ?></a>
        <?php } ?>
      <?php } ?>
      <a class="btn btn-app" href="PrintView.php?PersonID=<?= $per_ID ?>"><i class="fa fa-print"></i> <?= gettext("Hoja Imprimible") ?></a>
      <a class="btn btn-app" href="PersonView.php?PersonID=<?= $per_ID ?>&AddToPeopleCart=<?= $per_ID ?>"><i class="fa fa-cart-plus"></i> <?= gettext("Añadir al Carro") ?></a>
      <?php if ($_SESSION['bNotes']) { ?>
        <a class="btn btn-app" href="WhyCameEditor.php?PersonID=<?php echo $per_ID ?>"><i class="fa fa-question-circle"></i> <?= gettext("Notas Evaluación") ?></a>
        <a class="btn btn-app" href="NoteEditor.php?PersonID=<?php echo $per_ID ?>"><i class="fa fa-sticky-note"></i> <?= gettext("Añadir Nota") ?></a>
      <?php }
      if ($_SESSION['bDeleteRecords']) { ?>
        <a class="btn btn-app bg-maroon" href="SelectDelete.php?mode=person&PersonID=<?= $per_ID ?>"><i class="fa fa-trash-o"></i> <?= gettext("Borrar este Registro") ?></a>
      <?php }
      if ($_SESSION['bAdmin']) {
        $sSQL = "SELECT usr_per_ID FROM user_usr WHERE usr_per_ID = " . $per_ID;
        if (mysql_num_rows(RunQuery($sSQL)) == 0) { ?>
          <a class="btn btn-app" href="UserEditor.php?NewPersonID=<?= $per_ID ?>"><i class="fa fa-user-secret"></i> <?= gettext("Hacer Usuario") ?></a>
        <?php } else { ?>
          <a class="btn btn-app" href="UserEditor.php?PersonID=<?= $per_ID ?>"><i class="fa fa-user-secret"></i> <?= gettext("Editar Usuario") ?></a>
        <?php }
      } ?>
      <a class="btn btn-app" role="button" href="SelectList.php?mode=person"><i class="fa fa-list"></i> <?= gettext("Listado de Miembros") ?></span></a>
    </div>
  </div>
  <div class="col-lg-9 col-md-9 col-sm-9">
    <div class="nav-tabs-custom">
      ...
      <ul class="nav nav-tabs" role="tablist">
        <li role="presentation" class="active"><a href="#timeline" aria-controls="timeline" role="tab" data-toggle="tab"><?= gettext("Línea de Tiempo") ?></a></li>
        <li role="presentation"><a href="#family" aria-controls="family" role="tab" data-toggle="tab"><?= gettext("Familia") ?></a></li>
        <li role="presentation"><a href="#groups" aria-controls="groups" role="tab" data-toggle="tab"><?= gettext("Grupos Asignados") ?></a></li>
        <li role="presentation"><a href="#properties" aria-controls="properties" role="tab" data-toggle="tab"><?= gettext("Propiedades Asignadas") ?></a></li>
        <li role="presentation"><a href="#volunteer" aria-controls="volunteer" role="tab" data-toggle="tab"><?= gettext("Oportunidades de Servicio") ?></a></li>
      </ul>
            ...
            <div class="main-box-body clearfix">
                          ...
                          <?php if ($_SESSION['bManageGroups']) { ?>
                            ...
                            <div class="btn-group">
                              ...
                              <ul class="dropdown-menu" role="menu">
                                <li><a href="MemberRoleChange.php?GroupID=<?= $grp_ID ?>&PersonID=<?= $iPersonID ?>"><?= gettext("Cambiar Rol") ?></a></li>
                                <?php if ($grp_hasSpecialProps == 'true') { ?>
                                  <li><a href="GroupPropsEditor.php?GroupID=<?= $grp_ID ?>&PersonID=<?= $iPersonID ?>"><?= gettext("Actualizar Propiedades") ?></a></li>
                                <?php } ?>
                              </ul>
                            </div>
              ...
              if ($_SESSION['bManageGroups']) { ?>
                <div class="alert alert-info">
                  <h4><strong><?php echo gettext("Asignar a un Nuevo Grupo"); ?> </strong></h4>
                  <i class="fa fa-info-circle fa-fw fa-lg"></i> <span><?= gettext("Esta persona será añadida al nuevo grupo en el rol por defecto.") ?></span>
                  ...
                  <a href="#" onclick="GroupAdd()" class="btn btn-success" role="button"><?= gettext("Asignar a Este Grupo") ?></a>
                  <br>
                </div>
              <?php } ?>
            </div>
          ...
          <div class="main-box clearfix">
            <div class="main-box-body clearfix">
              ...
              if (mysql_num_rows($rsAssignedProperties) == 0) { ?>
                <br>
                <div class="alert alert-warning">
                  <i class="fa fa-question-circle fa-fw fa-lg"></i> <span><?= gettext("Sin propiedades asignadas.") ?></span>
                </div>
              <?php } else {
                //Yes, start the table
                echo "<table width=\"100%\" cellpadding=\"4\" cellspacing=\"0\">";
                echo "<tr class=\"TableHeader\">";
                echo "<td width=\"10%\" valign=\"top\"><b>" . gettext("Tipo") . "</b>";
                echo "<td width=\"15%\" valign=\"top\"><b>" . gettext("Nombre") . "</b>";
                echo "<td valign=\"top\"><b>" . gettext("Valor") . "</b></td>";

                if ($bOkToEdit) {
                  echo "<td valign=\"top\"><b>" . gettext("Editar") . "</b></td>";
                  echo "<td valign=\"top\"><b>" . gettext("Quitar") . "</b></td>";
                }
                echo "</tr>";
                ...
                while ($aRow = mysql_fetch_array($rsAssignedProperties)) {
                  ...
                  if ($bOkToEdit) {
                    if (strlen($pro_Prompt) > 0) {
                      echo "<td valign=\"center\"><a href=\"PropertyAssign.php?PersonID=" . $iPersonID . "&PropertyID=" . $pro_ID . "\">" . gettext("Editar") . "</a></td>";
                    } else {
                      echo "<td>&nbsp;</td>";
                    }
                    echo "<td valign=\"center\"><a href=\"PropertyUnassign.php?PersonID=" . $iPersonID . "&PropertyID=" . $pro_ID . "\">" . gettext("Quitar") . "</a></td>";
                  }
                  echo "</tr>";
              ...
              <?php if ($bOkToEdit && mysql_num_rows($rsProperties) != 0) { ?>
                <div class="alert alert-info">
                  <div>
                    <h4><strong><?= gettext("Asignar Nueva Propiedad:") ?></strong></h4>
                    ...
                    <form method="post" action="PropertyAssign.php?PersonID=<?= $iPersonID ?>">
                      ...
                      <input type="submit" class="btn-primary" value="<?= gettext("Asignar") ?>" name="Submit">
                    </form>
                  </div>
                </div>
              <?php } ?>
          ...
          <div class="main-box clearfix">
            <div class="main-box-body clearfix">
              ...
              if (mysql_num_rows($rsAssignedVolunteerOpps) == 0) { ?>
                <br>
                <div class="alert alert-warning">
                  <i class="fa fa-question-circle fa-fw fa-lg"></i> <span><?= gettext("No hay oportunidades de servicio asignadas.") ?></span>
                </div>
              <?php } else {
                echo "<table width=\"100%\" cellpadding=\"4\" cellspacing=\"0\">";
                echo "<tr class=\"TableHeader\">";
                echo "<td>" . gettext("Nombre") . "</td>";
                echo "<td>" . gettext("Descripción") . "</td>";
                if ($_SESSION['bEditRecords']) {
                  echo "<td width=\"10%\">" . gettext("Quitar") . "</td>";
                }
                echo "</tr>";
                ...
                while ($aRow = mysql_fetch_array($rsAssignedVolunteerOpps)) {
                  ...
                  if ($_SESSION['bEditRecords']) echo "<td><a class=\"SmallText\" href=\"PersonView.php?PersonID=" . $per_ID . "&RemoveVO=" . $vol_ID . "\">" . gettext("Quitar") . "</a></td>";
              ...
              <?php if ($_SESSION['bEditRecords']) { ?>
                <div class="alert alert-info">
                  <div>
                    <h4><strong><?= gettext("Asignar Nueva Oportunidad de Servicio:") ?></strong></h4>
                    ...
                    <form method="post" action="PersonView.php?PersonID=<?= $iPersonID ?>">
                      ...
                      <input type="submit" value="<?= gettext("Asignar") ?>" name="VolunteerOpportunityAssign" class="btn-primary">
                    </form>
                  </div>
                </div>
              <?php } ?>

...
// Constants
$aPropTypes = array(
    1 => gettext("Verdadero / Falso"),
    2 => gettext("Fecha"),
    3 => gettext("Campo Texto (50 car)"),
    4 => gettext("Campo Texto (100 car)"),
    5 => gettext("Campo Texto (largo)"),
    6 => gettext("Año"),
    7 => gettext("Estación"),
    8 => gettext("Número"),
    9 => gettext("Persona de Grupo"),
    10 => gettext("Dinero"),
    11 => gettext("Número Teléfono"),
    12 => gettext("Lista Desplegable Personalizada")
);
...
if (isset($_GET["AllPDFsEmailed"])) {
    $sGlobalMessage = gettext("PDFs enviados exitosamente ". $_GET["AllPDFsEmailed"] . " familias.");
}
    ...
    if ($Year > 0)
    {
        if ($Year == date("Y"))
        {
            ...
            if ($Day > date("d"))
                $monthCount--;
            if ($monthCount == 1)
                return (gettext("1 mes"));
            else
                return ( $monthCount . " " . gettext("meses"));
        }
        ...
        elseif ( $Month > date("m") || ($Month == date("m") && $Day > date("d")) )
            return ( date("Y")-1 - $Year . " " . gettext("años"));
        else
            return ( date("Y") - $Year . " " . gettext("años"));
    }
    else
        return ( gettext("Desconocida"));
}

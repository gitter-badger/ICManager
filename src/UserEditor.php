...
$sPageTitle = gettext('Editor de Usuario');
...
<div class="box">
    <div class="box-body">
        <div class="callout callout-info">
            <?= gettext('Nota: Los cambios no producirán efecto hasta el próximo inicio de sesión.') ?>
        </div>
...
?>
    <tr>
        <td><?= gettext('Persona a hacer usuario:') ?></td>
        <td>
...
<?php } ?>
    ...
    <?php } ?>
    <tr>
        <td><?= gettext('Nombre Login:') ?></td>
        <td><input type="text" name="UserName" value="<?= $sUserName ?>"></td>
    </tr>

    <tr>
        <td><?= gettext('Añadir Registros:') ?></td>
        <td><input type="checkbox" name="AddRecords" value="1"<?php if ($usr_AddRecords) { echo " checked"; } ?>></td>
    </tr>

    <tr>
        <td><?= gettext('Editar Registros:') ?></td>
        <td><input type="checkbox" name="EditRecords" value="1"<?php if ($usr_EditRecords) { echo " checked"; } ?>></td>
    </tr>

    <tr>
        <td><?= gettext('Borrar Registros:') ?></td>
        <td><input type="checkbox" name="DeleteRecords" value="1"<?php if ($usr_DeleteRecords) { echo ' checked'; } ?>></td>
    </tr>

    <tr>
        <td><?= gettext('Manejar Propiedades y Clasificaciones:') ?></td>
        <td><input type="checkbox" name="MenuOptions" value="1"<?php if ($usr_MenuOptions) { echo ' checked'; } ?>></td>
    </tr>

    <tr>
        <td><?= gettext('Manejar Grupos y Roles:') ?></td>
        <td><input type="checkbox" name="ManageGroups" value="1"<?php if ($usr_ManageGroups) { echo ' checked'; } ?>></td>
    </tr>

    <tr>
        <td><?= gettext('Manejar Donaciones y Finanzas:') ?></td>
        <td><input type="checkbox" name="Finance" value="1"<?php if ($usr_Finance) { echo ' checked'; } ?>></td>
    </tr>

    <tr>
        <td><?= gettext('Ver, Agregar y Editar Notas:') ?></td>
        <td><input type="checkbox" name="Notes" value="1"<?php if ($usr_Notes) { echo ' checked'; } ?>></td>
    </tr>

    <tr>
        <td><?= gettext('Edición a Sí Mismo:') ?></td>
        <td><input type="checkbox" name="EditSelf" value="1"<?php if ($usr_EditSelf) { echo ' checked'; } ?>>&nbsp;<span class="SmallText"><?= gettext('(Editar sólo su familia.)') ?></span></td>
    </tr>
    <tr>
        <td><?= gettext('Visitador:') ?></td>
        <td><input type="checkbox" name="Canvasser" value="1"<?php if ($usr_Canvasser) { echo ' checked'; } ?>>&nbsp;<span class="SmallText"><?= gettext('(Visitador voluntario.)') ?></span></td>
    </tr>
    <tr>
        <td><?= gettext('Administrador:') ?></td>
        <td><input type="checkbox" name="Admin" value="1"<?php if ($usr_Admin) { echo ' checked'; } ?>>&nbsp;<span class="SmallText"><?= gettext('(Entrega todos los privilegios.)') ?></span></td>
    </tr>
    <tr>
        <td><?= gettext('Estilo:') ?></td>
        <td class="TextColumnWithBottomBorder"><select name="Style"><?php StyleSheetOptions($usr_Style); ?></select></td>
    </tr>
    <tr>
        <td colspan="2" align="center">
        <input type="submit" class="btn btn-primary" value="<?= gettext('Guardar') ?>" name="save">&nbsp;<input type="button" class="btn" name="Cancel" value="<?= gettext('Cancelar') ?>" onclick="javascript:document.location='UserList.php';">
        </td>
    </tr>
</table>
    </div>

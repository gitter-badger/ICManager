...
//Set the page title
$sPageTitle = gettext("Vista de Familias");
...
// Format the phone numbers
  ...
  <div class="row">
    <div class="col-lg-3 col-md-4 col-sm-4">
      <div class="box box-primary">
        <div class="box-body">
          ...
          <h3 class="profile-username text-center"><?= gettext("La Familia") . " $fam_Name " ?></h3>
          <?php if ($bOkToEdit) { ?>
            <a href="FamilyEditor.php?FamilyID=<?= $fam_ID ?>" class="btn btn-primary btn-block"><b>Editar</b></a>
          <?php } ?>
          <hr/>
          <ul class="fa-ul">
            <li><i class="fa-li glyphicon glyphicon-home"></i>Dirección: <span>
            ...
            if ($fam_Latitude && $fam_Longitude) {
              if ($nChurchLatitude && $nChurchLongitude) {
                ...
                echo $sDistance . " " . strtolower($sDistanceUnit) . " " . $sDirection . " de la iglesia<br>";
              }
            }
            ?>
            <?php if (!$bHideLatLon) { /* Lat/Lon can be hidden - General Settings */ ?>
              <li><i class="fa-li fa fa-compass"></i><?= gettext("Latitud/Longitud") ?> <span><?= $fam_Latitude . " / " . $fam_Longitude ?></span></li>
            <?php }
            if (!$bHideFamilyNewsletter) { /* Newsletter can be hidden - General Settings */ ?>
              <li><i class="fa-li fa fa-hacker-news"></i><?= gettext("Enviar Boletín:") ?> <span><?= $fam_SendNewsLetter ?></span></li>
            <?php }
            if (!$bHideWeddingDate) { /* Wedding Date can be hidden - General Settings */ ?>
              <li><i class="fa-li fa fa-magic"></i><?= gettext("Fecha Matrimonio:") ?> <span><?= FormatDate($fam_WeddingDate, false) ?></span></li>
            <?php }
            if ($bUseDonationEnvelopes) { ?>
              <li><i class="fa-li fa fa-phone"></i><?= gettext("Número Sobre") ?> <span><?= $fam_Envelope ?></span></li>
            <?php }
            if ($sHomePhone != "") { ?>
              <li><i class="fa-li fa fa-phone"></i><?= gettext("Tel Casa:") ?> <span><?= $sHomePhone ?></span></li>
            <?php }
            if ($sWorkPhone != "") { ?>
              <li><i class="fa-li fa fa-building"></i><?= gettext("Tel Trabajo:") ?> <span><?= $sWorkPhone ?></span></li>
            <?php }
            if ($sCellPhone != "") { ?>
              <li><i class="fa-li fa fa-mobile"></i><?= gettext("Tel Móvil:") ?> <span><?= $sCellPhone ?></span></li>
            <?php }
            if ($fam_Email != "") { ?>
    ...
    <div class="col-lg-9 col-md-8 col-sm-8">
      <div class="row">
        <div class="box"><br/>
          <a class="btn btn-app bg-aqua-active" href="FamilyVerify.php?FamilyID=<?= $iFamilyID ?>"><i class="fa fa-check-square"></i> Verificar Info</a>
          <a class="btn btn-app bg-olive" href="PersonEditor.php?FamilyID=<?= $iFamilyID ?>"><i class="fa fa-plus-square"></i> Añadir Nuevo Miembro</a>
          <?php if (($previous_id > 0)) { ?>
            <a class="btn btn-app" href="FamilyView.php?FamilyID=<?= $previous_id ?>"><i class="fa fa-hand-o-left"></i> Familia Anterior</a>
          <?php } ?>
          <a class="btn btn-app btn-danger" role="button" href="FamilyList.php"><i class="fa fa-list-ul"></i> Lista de Familias</a>
          <?php if (($next_id > 0)) { ?>
            <a class="btn btn-app" role="button" href="FamilyView.php?FamilyID=<?= $next_id ?>"><i class="fa fa-hand-o-right"></i>Familia Siguiente </a>
          <?php } ?>
          <?php if ($_SESSION['bDeleteRecords']) { ?>
            <a class="btn btn-app bg-maroon" href="SelectDelete.php?FamilyID=<?= $iFamilyID ?>"><i class="fa fa-trash-o"></i> Borrar Familia</a>
          <?php } ?>
          <br/>
          <?php if ($bOkToEdit) { ?>
            <a class="btn btn-app" href="#" data-toggle="modal" data-target="#upload-image"><i class="fa fa-camera"></i> <?= gettext("Subir Fotografía") ?> </a>
          ...
          if ($_SESSION['bNotes']) { ?>
            <a class="btn btn-app" href="NoteEditor.php?FamilyID=<?= $iFamilyID ?>"><i class="fa fa-sticky-note"></i> Añadir Nota</a>
          <?php } ?>
          <a class="btn btn-app" href="Reports/ConfirmReport.php?familyId=<?= $iFamilyID ?>"><i class="fa fa-download"></i> Descargar Reporte PDF</a>
          <a class="btn btn-app" href="#" data-toggle="modal" data-target="#confirm-email-pdf"><i class="fa fa-send"></i> Enviar Reporte PDF (Email)</a>
          <a class="btn btn-app" href="FamilyView.php?FamilyID=<?= $iFamilyID ?>&AddFamilyToPeopleCart=<?= $iFamilyID ?>"> <i class="fa fa-cart-plus"></i> Añadir la Familia al Carro</a>
        </div>
      </div>
    </div>
        ...
        <div class="box box-solid">
          <div class="box-body table-responsive clearfix">
            <table class="table user-list table-hover">
              <thead>
              <tr>
                <th><span>Miembros Familia</span></th>
      ...
      <div class="nav-tabs-custom">
        ...
        <ul class="nav nav-tabs" role="tablist">
          ...
          <?php if ($_SESSION['bFinance']) { ?>
            <li role="presentation"><a href="#finance" aria-controls="finance" role="tab" data-toggle="tab"><?= gettext("Pagos Automáticos") ?></a></li>
            <li role="presentation"><a href="#pledges" aria-controls="pledges" role="tab" data-toggle="tab"><?= gettext("Compromisos y Pagos") ?></a></li>
          <?php } ?>
            ...
            <div class="main-box clearfix">
              <div class="main-box-body clearfix">
                ...
                <?php } else {
                  ...
                  echo "<td width=\"10%\" valign=\"top\"><b>" . gettext("Tipo") . "</b></td>";
                  echo "<td width=\"15%\" valign=\"top\"><b>" . gettext("Nombre") . "</b></td>";
                  echo "<td valign=\"top\"><b>" . gettext("Valor") . "</b></td>";

                  if ($bOkToEdit) {
                    echo "<td width=\"10%\" valign=\"top\"><b>" . gettext("Editar Valor") . "</td>";
                    echo "<td valign=\"top\"><b>" . gettext("Quitar") . "</td>";
                  }
                  ...
                  while ($aRow = mysql_fetch_array($rsAssignedProperties)) {
                    ...
                    if ($bOkToEdit) {
                      if (strlen($pro_Prompt) > 0) {
                        echo "<td valign=\"center\"><a href=\"PropertyAssign.php?FamilyID=" . $iFamilyID . "&amp;PropertyID=" . $pro_ID . "\">" . gettext("Editar Valor") . "</a></td>";
                      } else {
                        echo "<td>&nbsp;</td>";
                      }

                      echo "<td valign=\"center\"><a href=\"PropertyUnassign.php?FamilyID=" . $iFamilyID . "&amp;PropertyID=" . $pro_ID . "\">" . gettext("Quitar") . "</a></td>";
                    }
                  ...
                  <div class="alert alert-info">
                    <div>
                      ...
                      <form method="post" action="PropertyAssign.php?FamilyID=<?= $iFamilyID ?>">
                        ...
                        <input type="submit" class="btn btn-default" value="Asignar" name="Submit2" style="font-size: 8pt;">
                        </p>
                      </form>
                    </div>
                  </div>
                ...
                <?php if (mysql_num_rows($rsAutoPayments) > 0) { ?>
                  <table cellpadding="5" cellspacing="0" width="100%">
                    ...
                    <tr class="TableHeader">
                      <td><?= gettext("Tipo") ?></td>
                      <td><?= gettext("Próxima Fecha Pago") ?></td>
                      <td><?= gettext("Monto") ?></td>
                      <td><?= gettext("Intervalo (meses)") ?></td>
                      <td><?= gettext("Fondo") ?></td>
                      <td><?= gettext("Editar") ?></td>
                      <td><?= gettext("Borrar") ?></td>
                      <td><?= gettext("Actualizado") ?></td>
                      <td><?= gettext("Por") ?></td>
                    </tr>

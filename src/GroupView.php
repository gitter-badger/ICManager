...
//Set the page title
$sPageTitle = gettext('Vista de Grupo');
...
<div class="box">
    <div class="box-header with-border">
        <h3 class="box-title">Funciones de Grupo</h3>
    </div>
    <div class="box-body">
...
{
    echo '<a class="btn btn-app" href="GroupEditor.php?GroupID=' . $grp_ID . '"><i class="fa fa-pencil"></i>' . gettext('Editar Grupo') . '</a>';
    echo '<a class="btn btn-app bg-maroon" data-toggle="modal" data-target="#deleteGroup"><i class="fa fa-trash"></i>' . gettext('Borrar Grupo') . '</a>';
    ?>
    <!-- GROUP DELETE MODAL-->
            ...
            <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title" id="upload-Image-label"><?= gettext("Confirmar Borrado de Grupo") ?></h4>
                        </div>
                        <div class="modal-body">
                        <span style="color: red">
                           <?= gettext("Por favor confirma el borrado de este grupo:") ?>
                             ...
                             <p class="LargeError">
                                <?= gettext("Esto también borrará los roles y las propiedades asociadas a este grupo.") ?>
                            </p>
                            <?= gettext("Todas las membresías a este grupo y sus propiedades serán borradas. Los datos de los miembros no serán alterados.") ?>
                            <br><br>
                            <span style="color:black">Entiendo &nbsp;<input type="checkbox" name="chkClear"id="chkClear" ></span>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                            <button name="deleteGroupButton" id="deleteGroupButton" type="button" class="btn btn-danger" disabled><?= gettext("Borrar Grupo") ?></button>
                        </div>
                    </div>
            </div>
        </div>
     ...
     <div class="modal fade" id="changeMembership" tabindex="-1" role="dialog" aria-labelledby="deleteGroup" aria-hidden="true">
            <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                            <h4 class="modal-title" id="upload-Image-label">Cambiar Rol Miembro</h4>
                        </div>
                        <div class="modal-body">
                        <span style="color: red"><?= gettext("Por favor seleccione rol para miembro:") ?></span>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                            <button name="confirmMembershipChange" id="confirmMembershipChange" type="button" class="btn btn-danger"><?= gettext("Cambiar Membresía") ?></button>
                        </div>
                    </div>
            </div>
        </div>
    ...
    {
        echo '<a class="btn btn-app" href="GroupPropsFormEditor.php?GroupID=' . $grp_ID . '"><i class="fa fa-list-alt"></i>' . gettext('Editar Propiedades Específicas Grupo') . '</a>';
    }
}
echo '<a class="btn btn-app" href="GroupView.php?Action=AddGroupToCart&amp;GroupID=' . $grp_ID . '"><i class="fa fa-users"></i>' . gettext('Añadir Miembros al Carro') . '</a>';
echo '<a class="btn btn-app" href="GroupMeeting.php?GroupID=' . $grp_ID . '&amp;Name=' . $grp_Name . '&amp;linkBack=GroupView.php?GroupID=' . $grp_ID . '"><i class="fa fa-calendar-o"></i>' . gettext('Agendar Reunión') . '</a>';

echo '<a class="btn btn-app" href="MapUsingGoogle.php?GroupID=' . $grp_ID . '"><i class="fa fa-map-marker"></i>' . gettext('Mapear Grupo') . '</a>';
...
if ($sEmailLink)
{
    ...
    // Display link
     ?>
      <div class="btn-group">
        <a  class="btn btn-app" href="mailto:<?= mb_substr($sEmailLink,0,-3) ?>"><i class="fa fa-send-o"></i><?= gettext('Enviar Correo')?></a>
      ...
      <div class="btn-group">
        <a class="btn btn-app" href="mailto:?bcc=<?= mb_substr($sEmailLink,0,-3) ?>"><i class="fa fa-send"></i><?=gettext('Enviar Correo (CCO)') ?></a>
         <button type="button" class="btn btn-app dropdown-toggle" data-toggle="dropdown" >
          <span class="caret"></span>
          <span class="sr-only">Toggle Dropdown</span>
        </button>
...
if ($sPhoneLink)
{
    ...
    // Display link
    echo '<a class="btn btn-app" href="javascript:void(0)" onclick="allPhonesCommaD()"><i class="fa fa-mobile-phone"></i> Enviar SMS</a>';
...
<div class="box">
    <div class="box-header with-border">
        <h3 class="box-title">Propiedades de Grupo</h3>
    </div>
    <div class="box-body">
...
<tr>
        ...
        <div class="LightShadedBox">
            ...
            <table width="98%">
                <tr>
                    ...
                    <?= gettext('Número Miembros:') ?> <?= $iTotalMembers ?>
                    <br>
                    <?= gettext('Tipo de Grupo:') ?> <?= $sGroupType ?>
                    <br>
                    <?= gettext('Rol por Defecto:') ?> <?= $sDefaultRole ?>
                    </font></div></td>
                </tr>
            </table>
        </div>
    ...
    <b><?= gettext('Propiedades Específicas de Grupo:') ?></b>
    ...
    if ($grp_hasSpecialProps == 'true')
    {
        ...
        if (!$numRows)
        {
            echo '<p>Ninguna propiedad ha sido creada</p>';
        }
        else
        {
            ?>
        ...
        echo '<p>' . gettext('Desactivadas para este grupo.') . '</p>';

    //Print Assigned Properties
    echo '<br>';
    echo '<b>' . gettext('Propiedades Asignadas:') . '</b>';
    $sAssignedProperties = ',';
    ...
    if (mysql_num_rows($rsAssignedProperties) == 0)
    {
        ...
        echo '<p>' . gettext('No hay propiedades asignadas.') . '</p>';
    }
    else
    {
        ...
        <table width="100%" cellpadding="2" cellspacing="0">
        <tr class="TableHeader">
        <td width="15%" valign="top"><b><?= gettext('Tipo') ?></b>
        <td valign="top"><b><?= gettext('Nombre') ?></b>
        <td valign="top"><b><?= gettext('Valor') ?></td>
        <?php

        if ($_SESSION['bManageGroups'])
        {
            echo '<td valign="top"><b>' . gettext('Editar Valor') . '</td>';
        ...
        while ($aRow = mysql_fetch_array($rsAssignedProperties))
        {
            ...
            {
                echo '<td valign="top"><a href="PropertyAssign.php?GroupID=' . $iGroupID . '&amp;PropertyID=' . $pro_ID . '">' . gettext('Editar Valor') . '</a></td>';
            }
            else
            {
                echo '<td>&nbsp;</td>';
            ...
            if ($_SESSION['bManageGroups'])
            {
                echo '<td valign="top"><a href="PropertyUnassign.php?GroupID=' . $iGroupID . '&amp;PropertyID=' . $pro_ID . '">' . gettext('Quitar') . '</a>';
            }
            else
            {
                echo '<td>&nbsp;</td>';
    ...
    if ($_SESSION['bManageGroups'])
    {
        ...
        echo '<p>';
        echo '<span>' . gettext('Asignar Nueva Propiedad:') . '</span>';
        ...
        echo '</select>';
        echo '<input type="submit" class="btn btn-default" value="' . gettext('Asignar') . '" name="Submit" style="font-size: 8pt;">';
        echo '</p></form>';
    }
    else
    {
...
<div class="box">
    <div class="box-header with-border">
        <h3 class="box-title"><?= gettext('Miembros del Grupo:') ?></h3>
    </div>
    <div class="box-body">
...
<form action="#" method="get" class="sidebar-form">
    <label for="addGroupMember"><?= gettext("Añadir Miembro al Grupo: ") ?></label>
    ...
    columns: [
       {
        ...
        },
        {
            ...
            render: function (data,type,full,meta) {
                return '<button type="button" class="btn btn-danger removeUserGroup" id="rguid-'+full.per_ID+'">Quitar Usuario del Grupo</button>';
            }
        }
    ]
    });
